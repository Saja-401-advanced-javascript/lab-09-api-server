...
describe('Products API', () => {
  it('post a new product', () => {
    let obj = { name: 'testCategory', category: 'cool', display_name: 'cool-category', description: 'not for sale', };
    ...
    return mockRequest.post('/api/v1/products')
      ...
      .then(data => {
        return mockRequest.put(`/api/v1/products/${data.body._id}`)
          .send({ name: 'testProductUpdated', category: 'cool', display_name: 'cool-category', description: 'not for sale', })
          .then(results => {
            expect(results.status).toBe(200);
